 ...
 * @author $author$
 */
public class ImageButton extends ImageTextLabel implements MouseListener,
		ItemSelectable {
	...
	public ImageButton(Image image, String text, String actionCommand,
			int hoverBorder, int depressedBorder, int normalBorder) {
		super(image, text);
	...
	public void setHoverButton(boolean hoverButton) {
		...
		setBorderType(hoverButton ? (mouseInComponent ? hoverBorder
				: normalBorder) : hoverBorder);

	}

	/**
	 * Set the background color to use when hovering over a button.
	 * <code>null</code> means use default color.
	 * 
	 ...
	 * = this.createImage(producer); int border = this.getBorder(); if
	 * (getScale() == STRETCH) this.prepareImage(grayImage, getWidth() - 2 *
	 * border, getHeight() - 2 * border, this); else
	 * this.prepareImage(grayImage, this); super.paint(g); }
	 */
