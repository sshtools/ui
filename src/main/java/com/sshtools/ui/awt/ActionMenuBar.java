 ...
 * @author $Autho$
 */
public class ActionMenuBar extends Panel implements FocusListener,
	...
	class MenuCanvas extends Canvas implements MouseMotionListener,
			MouseListener {
		...
		public void paint(Graphics ig) {
			...
			if (backingStore == null
					|| (backingStore != null && (s.width != backingStore
							.getWidth(this) || s.height != backingStore
							.getHeight(this)))) {
				if (backingStore != null) {
			...
			if (menu != null) {
				...
				for (Enumeration e = menu.children(); e.hasMoreElements();) {
					...
					if (m.getName().equals(ActionMenu.SEPARATOR.getName())) {
						...
						if (m.isEnabled()) {
							if (i == sel) {
								...
								int ty = borderWidth
										...
										+ (py - borderWidth - insets.top);
								g.fillRect(borderWidth, ty, s.width
										- (borderWidth * 2), itemHeight);
								if (borderWidth != 0) {
									g.setColor(borderColor);
									g.drawLine(borderWidth, ty,
											(borderWidth * 2) + s.width - 1, ty);
									g.drawLine(borderWidth,
											ty + itemHeight - 1,
											(borderWidth * 2) + s.width - 1, ty
													+ itemHeight - 1);
								}
						...
						int toff = 0;
						if (imagePath != null) {
							Image img = UIUtil.loadImage(m.getClass(),
									imagePath);
							if (!m.isEnabled()) {
								img = createImage(new FilteredImageSource(
										img.getSource(), new GrayFilter()));
								UIUtil.waitFor(img, this);
							}
					...
					if (tip != null) {
						...
						if (toolTipsEnabled) {
							ToolTipManager.getInstance()
									.requestToolTip(this, p.x + e.getX() + 4,
											p.y + e.getY() + 4, tip);
						}
					}
				} else {
			...
			if ((e.getModifiers() & MouseEvent.BUTTON1_MASK) != 0) {
				...
				if (clicked != -1 && clicked < menu.getChildCount()) {
					...
					action.actionPerformed(new ActionEvent(this,
							ActionEvent.ACTION_PERFORMED, action.getName(), e
									.getModifiers()));
				}
			}
			hideMenuWindow();
