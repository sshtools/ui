 ...
 *
 * <p>
 * Each component added except the first will be invisible until the user clicks
 * on the tab heading either above, below. to the left, or to the right of the
 * visible component. Clicking on the heading will hide the current tab and make
...
public class TabbedPanel extends Container {
	...
	/**
	 * <p>
	 * Create a tabbed panel with the tabs at the specified position. Can be one
	 * of :-
	 * </p>
	 * <ul>
	 ...
	 *
	 * <p>
	 * <b><font color="#ff0000">Note, only <i>TOP</i> is currently
	 * supported</font></b>
	 * </p>
	 *
	 * @param position
		...
		addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent evt) {
				for (Enumeration e = tabs.elements(); e.hasMoreElements();) {
					...
					if (w.bounds != null
							&& w.bounds.contains(evt.getX(), evt.getY())) {
	...
	/**
	 * Add an <code>ActionListener</code> to be informed when the user selects a
	 * tab.
	 *
	 * @param l
	 *            listener to add
	...
	public void setSelectedTab(int idx) {
		...
		if (sel != -1) {
			...
			for (int i = s - 1; i >= 0; i--) {
				if (aevt == null) {
					aevt = new ActionEvent(TabbedPanel.this,
							ActionEvent.ACTION_PERFORMED, ""); //$NON-NLS-1$
				}
				((ActionListener) listenerList.elementAt(i))
						.actionPerformed(aevt);
			}
			Component c = getComponent(sel);
			TabWrapper t = (TabWrapper) tabs.get(c);
			if (t != null) {
				layout.show(this, t.name);
			}
			repaint();
		}
	}
	...
	public void add(Component comp, Object constraints, int index, Image image) {
		TabWrapper w = new TabWrapper(comp.getName(),
				String.valueOf(constraints), image, comp, constraints);
		tabs.put(comp, w);
		...
		case TOP:
			...
			for (int i = 0; i < ncomponents; i++) {
				...
				if (tab != null) {
					...
					int hw = metrics.stringWidth(tab.text)
							+ (HORIZONTAL_GAP * 2);
			...
			if (selx != -1) {
				...
				g.setColor(getForeground());
				g.drawString(tab.text, x + HORIZONTAL_GAP, (r.height / 2)
	...
	private Dimension getHeadingSize() {
		...
		for (int i = 0; i < c; i++) {
			...
			TabWrapper tab = (TabWrapper) tabs.get(comp);
			if (tab != null) {
				int thw = (HORIZONTAL_GAP * 2)
						+ (metrics != null ? metrics
								.stringWidth(tab.text == null ? "" : tab.text) : 0) //$NON-NLS-1$
						+ (tab.image == null ? 0 : (IMAGE_TEXT_GAP + tab.image
								.getWidth(this)));
				int thh = (VERTICAL_GAP * 2)
						+ Math.max(
								metrics != null ? metrics.getHeight() : 0,
								tab.image == null ? 0 : tab.image
										.getHeight(this));
				...
				/* DEBUG */System.err
						.println(Messages
								.getString("TabbedPanel.tabWrapperCouldNotBeFound") + i + " [" + comp + //$NON-NLS-1$ //$NON-NLS-2$
	...
	class TabbedLayout extends CardLayout {
		...
		public void layoutContainer(Container parent) {
			synchronized (parent.getTreeLock()) {
				...
				int headingSize = position == TOP || position == BOTTOM ? s.height
						: s.width;
				insets = new Insets(insets.top
						+ (position == TOP ? headingSize : 0), insets.left
						+ (position == LEFT ? headingSize : 0), insets.bottom
						+ (position == BOTTOM ? headingSize : 0), insets.right
						+ (position == BOTTOM ? headingSize : 0));
				for (int i = 0; i < ncomponents; i++) {
		...
		public Dimension minimumLayoutSize(Container parent) {
			...
			return position == TOP || position == BOTTOM ? new Dimension(
					Math.max(s.width, c.width), s.height + c.height)
					: new Dimension(s.width + c.width, Math.max(s.height,
							c.height));
		}
