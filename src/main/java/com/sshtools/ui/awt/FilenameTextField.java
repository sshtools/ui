...
public class FilenameTextField extends Panel {
	...
	public FilenameTextField(String text, int columns, Image chooserButtonImage) {
		this(text, columns, chooserButtonImage, Messages
				.getString("FilenameTextField.selectAFile")); //$NON-NLS-1$
	}

	public FilenameTextField(String text, int columns,
			Image chooserButtonImage, String chooserToolTipText) {
		super(new BorderLayout(4, 1));
		chooserButton = new ImageButton(chooserButtonImage, text,
				Messages.getString("FilenameTextField.browse")); //$NON-NLS-1$
		...
		textField = new TextField(text, columns) {
			public Dimension getPreferredSize() {
				return new Dimension(super.getPreferredSize().width,
						chooserButton.getPreferredSize().height);
			}

			public Dimension getMinimumSize() {
				return new Dimension(super.getMinimumSize().width,
						chooserButton.getMinimumSize().height);
			}
		};
	...
	private void createChooser() {
		if (chooser == null) {
			...
			chooser = new FileSelect(FileSelect.FILES_AND_DIRECTORIES, cwd,
					true, true);
		}
	}
	...
	private void showChooser() {
		...
		Option option = chooser.showDialog(this,
				Messages.getString("FilenameTextField.selectAFile")); //$NON-NLS-1$
