...
public class ImageTextLabel extends Canvas {
	...
	public void paint(Graphics g1) {
		...
		if (buffer == null || d.width != buffer.getWidth(this)
				|| d.height != buffer.getHeight(this) || wasEnabled != enabled) {
			if (buffer != null) {
			...
			try {
				...
				// Its possible the graphics cannot be created yet, just ignore
				// and hope that they can be on the next repaint
				return;
			}
		}
		wasEnabled = enabled;
	...
	protected void renderComponent(Graphics g, Dimension d, boolean enabled) {
		...
		// Draw the image
		if (imageX != -1) {
			g.drawImage(enabled ? image : grayImage, offx + imageX,
					(d.height - image.getHeight(this)) / 2, this);
		}
		...
		if (textX != -1) {
			...
			g.drawString(text, offx + textX,
					metrics.getHeight() - metrics.getDescent()
							+ ((d.height - metrics.getHeight()) / 2));

			// g.drawString(text, offx + textX, ((d.height - i.bottom) / 2 +
			// metrics.getAscent() / 2));
		}
		paintBorder(g);
	}
	...
	public void setImage(Image image) {
		...
		if (image != null) {
			...
			if (source != null) {
				grayImage = createImage(new FilteredImageSource(source,
						new GrayFilter()));
	...
	/**
	 * Set the border type. Can be one of :- </p>
	 * 
	 * <ul>
	...
	public Insets getInsets() {
		Insets i = borderType == NONE ? new Insets(0, 0, 0, 0) : new Insets(2,
				2, 2, 2);
		if (margin != null) {
			i.top += margin.top;
	...
	public Dimension getPreferredSize() {
		...
		return new Dimension(
				i.left
						+ i.right
						+ 2
						+ (image != null && text != null && textVisible ? textImageGap
								: 0)
						+ (image != null ? image.getWidth(this) : 0)
						+ (metrics != null && textVisible && text != null ? metrics.stringWidth(text)
								: 0), i.top
						+ i.bottom
						+ Math.max(image != null ? image.getHeight(this) : 0,
