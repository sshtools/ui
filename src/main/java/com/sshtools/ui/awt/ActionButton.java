...
/**
 * A utility class that extends {@link ImageButton} to create a button from an
 * {@link Action}.
 *
 * @author $Author: brett $
 * @author $Revision: 1.1.2.1 $
 */
...
public class ActionButton extends ImageButton implements PropertyChangeListener {
	...
	/**
	 * Set the image, text, action listener etc from the action. Any previous
	 * action will be deregistered.
	 *
	 * @param action
	 *            action
	 */
	public void setAction(final Action action) {
		...
		this.action = action;
		String imgName = LARGE_ICONS.equals(iconDisplay) ? (String) action
				.getValue(Action.IMAGE_PATH)
				: (SMALL_ICONS.equals(iconDisplay) ? (String) action
						.getValue(Action.SMALL_IMAGE_PATH) : null);
		Image img = null;
		if (imgName != null) {
		...
		// Also show text if icons were requested but this one is not available
		// - prevents empty buttons
		setTextVisible(img == null
				|| SHOW_TEXT.equals(textDisplay)
				|| (SELECTIVE_TEXT.equals(textDisplay) && (hide == null || !hide
						.booleanValue())));
