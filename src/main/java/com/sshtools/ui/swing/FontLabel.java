...
public class FontLabel extends JTextField {
	...
	public void setChosenFont(Font f) {
		setFont((f == null || f.getName().equals(FontChooser.AUTOMATIC) ? UIManager
				.getFont("Label.font") : f)
				.deriveFont((float) fixedPreviewSize));
		this.chosenFont = f;
		rebuildText();
	}
	...
	void rebuildText() {
		if (chosenFont != null
				&& !chosenFont.getName().equals(FontChooser.AUTOMATIC)) {
			setText(chosenFont.getName()
					+ (showSize ? ("," + chosenFont.getSize() + "pt") : ""));
		} else {
			setText("Automatic");
		}
	}
	...
	public Font getChosenFont() {
		return chosenFont == null
				|| chosenFont.getName().equals(FontChooser.AUTOMATIC) ? null
				: chosenFont;
	}
}
